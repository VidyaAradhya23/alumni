...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, TextInput, SafeAreaView, StatusBar, Alert, ScrollView, Modal, Image } from 'react-native';
import { Ionicons } from '@expo/vector-icons';

const JobsScreen = ({ navigation, route }) => {
  const [activeTab, setActiveTab] = useState('post_job'); // 'post_job' | 'preferences'
  ...
  // Reuse the preference modal for work mode
  ...
  const [resumeList] = useState([
    ...
    { 
      ...
      desc: 'over 11 years of experience in Business Development, Key Account Management, Contract Project Management, P&L Management and Team Management.',
    },
  ]);
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.editorHeader}>
          ...
          <View style={[styles.editorHeaderRight, { width: 24 }]} />
        </View>
        ...
        <View style={styles.jobDetailsBottomBar}>
          <TouchableOpacity style={styles.jobDetailsApplyBtn} onPress={() => Alert.alert('Applied', 'You have successfully applied for this role.')}>
  ...
  if (showResumeBook) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.editorHeader}>
          <TouchableOpacity onPress={() => setShowResumeBook(false)}>
            <Ionicons name="arrow-back" size={24} color="#0F172A" />
          </TouchableOpacity>
          <View style={styles.editorHeaderCenter}>
              ...
              <View style={styles.resumeCardTop}>
                ...
                <View style={styles.resumeDetailsSection}>
                  ...
                  <View style={styles.resumeDetailRow}>
                    ...
                    <Text style={styles.resumeCompanyText}>{item.company} - <Text style={styles.resumeRoleTextLight}>{item.role}</Text></Text>
                  </View>

                  {/* Domain & Experience */}
                  <View style={styles.resumeDetailRow}>
                    <Ionicons name="build-outline" size={16} color="#94A3B8" style={{marginRight: 8}} />
                    <Text style={styles.resumeDomainText}>{item.domain} <Text style={styles.resumeRoleTextLight}>{item.experience}</Text></Text>
                  </View>

                  {/* Description if exists */}
                  {item.desc ? (
                    <Text style={styles.resumeDescText}>{item.desc}</Text>
                  ) : null}
                </View>
              </View>
        ...
        <Modal visible={workModeModalVisible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            ...
            <View style={styles.modalContent}>
              ...
              {['Full-Time', 'Part-Time', 'Internship', 'Contract', 'Hybrid', 'Remote'].map((mode) => (
                <TouchableOpacity
                  key={mode}
                  style={[
                    styles.modalItem,
                    jobWorkMode === mode && styles.selectedModalItem
                  ]}
                  onPress={() => {
                    setJobWorkMode(mode);
                    setWorkModeModalVisible(false);
                  }}
                >
                  <Text style={[
                    styles.modalItemText,
                    jobWorkMode === mode && styles.selectedModalItemText
                  ]}>
                    {mode}
                  </Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabBar}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'post_job' && styles.activeTabText]}>
            Post Job
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
      ...
      {activeTab === 'post_job' ? (
        <View style={{ flex: 1 }}>
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubtitle}>Tap the floating button below to post a new job opportunity.</Text>
              </View>
            }
          />
          ...
          <View style={styles.fabContainer}>
            <TouchableOpacity style={styles.fabSmall} onPress={() => setShowEditor(true)} activeOpacity={0.85}>
              <Ionicons name="add" size={24} color="#FFFFFF" />
            </TouchableOpacity>

            <TouchableOpacity style={[styles.extendedFab, { marginTop: 12 }]} onPress={() => setShowResumeBook(true)} activeOpacity={0.85}>
          ...
          {profileMatches.map((match) => (
            <TouchableOpacity 
              ...
              onPress={() => setSelectedJob({
                ...
                description: `This is a ${match.match} for your profile. As a ${match.title} at ${match.company}, you will be responsible for building scalable systems and contributing to the core team.`
              })}
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.commTitle}>{match.title}</Text>
                <Text style={styles.commDesc}>{match.company} • {match.location}</Text>
                <Text style={styles.matchScoreText}>{match.match}</Text>
              </View>
